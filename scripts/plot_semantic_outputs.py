...
def main() -> int:
    ...
    runs = discover_runs(input_dir, args.prefix)
    if not runs:
    ...
    for prefix, files in runs.items():
        ...
        label_col = args.label_column or infer_label_column(scores)
        outlier_label_map = build_outlier_label_map(
            scores=scores,
            outlier_col=args.outlier_column,
            label_col=label_col,
            fallback_count=args.max_heatmap_rows,
        )
        ...
        plot_pc_z_heatmap(
            ...
            args.outlier_column,
            outlier_label_map,
            args.max_heatmap_rows,
            outdir / "{}_pc_z_heatmap.pdf".format(prefix),
            absolute=args.absolute_heatmap,
        )
