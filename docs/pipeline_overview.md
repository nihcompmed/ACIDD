...
```
`*` The `analyze-file` CLI **requires** scales and weights (see §4); pan-mild is
optional. The library `analyze_survey_table` can still run without them (item
inference, global sentinels, observed ranges, unweighted statistics) for tests
and flexibility, but the CLI does not expose that path.
...
> **Required vs optional.** The `analyze-file` CLI **requires** a scale file (per-item
> `reverse` flag — part of the method) **and** a weights file. For an unweighted
> analysis (as in the paper), pass a column of **equal weights** (e.g. all `1`s):
> equal weights make WLS = OLS, and the weighted Mahalanobis becomes the unweighted
> distances times a constant `sqrt((n-1)/n)` — so the **outlier ranking and empirical
> outlier set are identical** (only absolute distances differ, negligibly). Pan-mild stays
> optional. The library `analyze_survey_table` still accepts no `item_scales` /
> no `sample_weights` (for flexibility and tests); only the CLI enforces them.
...
python -m survey_semantics.cli analyze-file path/to/data.csv \
  ...
  --scale-file   path/to/data_scales.csv \    # required
  --weights-file path/to/data_weights.csv \   # required (all-1s for unweighted)
  --id-col       <ID_COLUMN> \
